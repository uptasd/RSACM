...
public class GatherCheckReply extends Behaviour {
    ...
    //    int unHitRes = 0;
    int updated_num = 0;
    Set<String> updatedAgent = new HashSet<>();
    AtomicInteger last_serviceNum;
    int noiseRate;
    ...
    public GatherCheckReply(int[] nums, AtomicInteger serviceNum, ACLMessage checkMsg,
            ...
            Map<String, ArrayList<String>> replacementMap, Map<String, Integer> updatedFunctions,
            AtomicInteger last_serviceNum,int noiseRate) {
...
//                    replyMsg.setUpdatedAgent(updatedAgent);
...
//                    }
                        ...
                        Set<String> set_ = answerCheckMsg.getUpdatedAgent();
//                        System.out.println(answerCheckMsg.isInitFlag());
                        CheckMessage requireMSG;
                        try {
                            requireMSG = mapper.readValue(requireCheckMessage.getContent(), CheckMessage.class);
                            int period = requireMSG.getPeriod();
                            if (set_ != null && set_.size() != 0 && !requireMSG.isInitFlag()) {
                                updated_num += answerCheckMsg.getUpdatedNum();
                                updatedAgent.addAll(set_);
//                                System.out.println(period + myAgent.getLocalName() + "收到状态变化的节点");
                            }
                        } catch (IOException e) {
                            throw new RuntimeException(e);
                        }
                        ...
                        if (relationType.equals("add")) {
                            ...
                            //通过依赖，组合，聚合关系连接的功能涉及到的资源数量
                            int resNum = answerCheckMsg.getRequiredResNum();
                            totalRes += resNum;
//                            int unHitNum = answerCheckMsg.getUnHitResNum();
//                            unHitRes += unHitNum;
                        }
...
//                        System.out.println(myAgent.getLocalName() + "-min:" + min);
                            //只是把下面的更新状态注释掉
                            try {
                                CheckMessage ask = mapper.readValue(requireCheckMessage.getContent(),
                                                                    CheckMessage.class);
                                boolean init = ask.isInitFlag();
                                if (init) {
                                    serviceNum.set(min);
//                                    if (myAgent.getLocalName().equals("中山大学第三附属医院-医院功能")) {
...
//                                    System.out.println("[替换视图]" + rep + "正在添加替换:" + myName);
                    ...
                    if (checkMode.equals("global")) {
                        int old_num = last_serviceNum.get();
                        int new_num = serviceNum.get();
                        if (new_num != old_num) {
//                            if(myName.equals("输血科-护士值班")){
//                                System.out.println(myName + "-功能节点状态变化,last:" + old_num + ",new:" + new_num);
//                            }
                            updatedAgent.add(myAgent.getLocalName());
                            updated_num++;
                        }
                        last_serviceNum.set(new_num);
                    }
                    ...
                    replyCheckMsg.setRequiredResNum(totalRes);
                    replyCheckMsg.setUpdatedNum(updated_num);
                    replyCheckMsg.setUpdatedAgent(updatedAgent);
//                    replyCheckMsg.setInitFlag(requireCheckMsg.isInitFlag());
//                        replyCheckMsg.setUnHitResNum(unHitRes);
                    try {
...
//                    }
                    ...
                    if (myName.equals("中山大学第三附属医院-网络服务") && checkMode.equals("global")) {
                        String dir = JadeUtil.outputPath;
                        String fileName = "中山大学第三附属医院-网络服务-"+noiseRate+".txt";
                        append2file(dir + fileName,
                                    period + ":" + serviceNum.get() + "\n");
                        System.out.println(myName + ":" + serviceNum.get());
                    }
                    if (myName.equals("中山大学第三附属医院-服务大厅功能") && checkMode.equals("global")) {
                        String dir = JadeUtil.outputPath;
                        String fileName = "中山大学第三附属医院-服务大厅功能-"+noiseRate+".txt";
                        append2file(dir + fileName,
                                    period + ":" + serviceNum.get() + "\n");
                        System.out.println(myName + ":" + serviceNum.get());
                    }
