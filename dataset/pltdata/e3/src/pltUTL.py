...
class PltUtil:
    ...
    def plt_error_rate(self):
        error_rate_data = self.read_error_rate_data()
        period = range(1, len(error_rate_data) + 1)
        error_rate = error_rate_data
        fig = plt.figure(figsize=(900 / 72, 400 / 72))
        plt.plot(period, error_rate_data, linewidth=1, color="black", marker="o", label="Error Rate")
        plt.legend(["Error Rate"], loc="upper left")
        plt.xlabel("period", self.font2)
        plt.ylabel("Error Rate", self.font2)
        print(max(error_rate))
        plt.xticks(fontsize=self.fontsize)
        plt.yticks(fontsize=self.fontsize)
        plt.savefig(self.save_path + "output/error-rate.png")
