...
//                        possibleNum++;
                    ...
                    if (replyCnt >= sendNum) {
                        ...
                        if (succeedFailAgentSet.size() >= RepFunctions.size()) {
                            ...
                            int urgencyLevel = 0;
                            String requiredQOS = "low-cost";
                            String rqID = "";
                            try {
                                AvailableMessage ask_available_FromUpMsg = mapper.readValue(
                                        askAvailableMsgFromUp.getContent(), AvailableMessage.class);
                                urgencyLevel = ask_available_FromUpMsg.getUrgencyLevel();
                                requiredQOS = ask_available_FromUpMsg.getRequiredType();
                                rqID = ask_available_FromUpMsg.getRequirementID();
                            } catch (IOException e) {
                                throw new RuntimeException(e);
                            }
...
//            selectedAggAID.setName(selectedAggName);
                            ...
                            //避免将容量减少到负数，将原本为0的容量临时设置为1，之后即时释放后变为1，自检过程中会修正到正确的容量
                            serviceNum.set(1);
                            step = 1;
                        } else {
